...
```text
--config PATH      Path to an explicit .env file (overrides ./.env discovery).
...
Configuration is **100% environment-variable driven**. There is no TOML or YAML config file. Local overrides live in a gitignored `.env` file at the project root. Every supported variable is documented in [`.env.sample`](.env.sample) with its default value.

### Precedence (lowest → highest)

1. Built-in defaults (in `src/saddleback/config.py`)
2. Variables already exported in your shell
3. Variables loaded from `.env` (only applied to keys not already in the environment — shell wins over file)

You can point at an explicit env file instead of `./.env` with `saddleback --config /path/to/my.env <subcommand>`.

### Quick setup

```bash
# Copy the template, then edit the few values that matter for you.
cp .env.sample .env
$EDITOR .env
```

Most operators only need to change `SADDLEBACK_TRANSLATOR_ENDPOINT` (if LM Studio runs on a non-loopback host). Everything else has a sane default.

### Common ad-hoc overrides

```bash
# One-shot, no file edit:
SADDLEBACK_TRANSLATOR_ENDPOINT=http://10.0.0.5:1234/v1 saddleback dub meeting.mp4

# Or export for the shell session:
export SADDLEBACK_TRANSCRIBE_MODEL=large-v3        # pin if turbo regresses
export SADDLEBACK_RUNTIME_TEST_MODE=true            # deterministic dry-run
...
│   ├── config.py               # env-var-driven config with .env loader (pydantic)
...
5. If the host has changed, set `SADDLEBACK_TRANSLATOR_ENDPOINT` in your shell or `.env`.
