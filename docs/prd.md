...
- **Privacy by construction.** No external HTTP egress is permitted by design for any pipeline stage in MVP. The translator endpoint is on the operator's LAN (`http://192.168.0.42:1234`). Whisper, mlx_audio, and ffmpeg are all on-device. Any future cloud option must be a deliberate, opt-in addition.
